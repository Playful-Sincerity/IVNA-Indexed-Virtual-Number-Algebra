# IVNA Phase 4: Unification Thesis Pivot

## Previous Phase Status
Phase 3 completed the following (see archived plan below for details):
- ✅ Algebraic characterization: IVNA ≅ K* × Z (Laurent monomials)
- ✅ Blow-up correspondence: exact match, neither subsumes the other
- ✅ Recursive indexing closure: C\{0} index domain, virtual indices reduce to order changes
- ✅ Outreach system: 4,692 researchers, templates, n8n workflow spec
- ⬜ External theorem: candidates explored (Bézout abandoned, ODE confluence abandoned, blow-up theorem in progress)
- ⬜ Collaborator: system built but no engagement yet

## Phase 4 Goal
**Pivot the paper from "calculus tool" framing to "cross-domain unification" thesis.**

A deep dive (2026-04-01) discovered that the product rule 0_x · ∞_y = xy appears across 9 mathematical domains — each of which independently invented heavy machinery to resolve the same indeterminate forms. The strongest new finding: **A8 (0_a/0_b = a/b) IS Bayes' theorem for continuous densities**, dissolving the Borel-Kolmogorov paradox without measure theory.

This changes the paper's argument from "here's a new notation for calculus" to "here's a single algebraic rule that unifies how 9 domains resolve indeterminacy." The calculus applications become supporting examples, not the thesis.

## Environment Health
**WARNINGS** — Uncommitted changes from debate + deep dive. Commit before starting parallel work.
...
### Sessions (5 workstreams)

| # | Session | Brief File | Can Start | Depends On |
|---|---------|-----------|-----------|------------|
| 1 | **VERIFY** — Triple-check all unification claims | `plan-session-verify.md` | NOW | — |
| 2 | **SEARCH** — Literature & prior art for each claim | `plan-session-search.md` | NOW | — |
| 3 | **EXPLORE** — More deep dives into new territory | `plan-session-explore.md` | NOW | — |
| 4 | **RESTRUCTURE** — Paper edit plan | `plan-session-restructure.md` | After 1+2 | VERIFY, SEARCH |
| 5 | **DEBATE** — Adversarial stress-test of new thesis | `plan-session-debate.md` | After 4 | RESTRUCTURE |

**Start 3 concurrent conversations:** Sessions 1, 2, and 3 are fully independent. Paste the brief file contents into each new conversation. When all three complete, start Session 4. When Session 4 completes, start Session 5.
...
VERIFY  ──────────┐
                   ├──→ RESTRUCTURE ──→ DEBATE
SEARCH  ──────────┘
EXPLORE (independent, feeds into RESTRUCTURE if HIGH-rated findings)
```

### Overall Success Criteria
1. Every unification claim has ≥2 independent verifications (VERIFY)
2. Every claim has a prior art assessment (SEARCH)
3. ≥3 new verified findings (EXPLORE)
4. Paper restructure plan approved by Wisdom (RESTRUCTURE)
5. Debate finds no fatal flaws in the unification thesis (DEBATE)

---

## Phase 3 Archive (completed work below)
